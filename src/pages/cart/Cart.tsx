...
const Cart = () => {
  ...
  const subtotal = cart.reduce((acc, product) => {
    const discount = Number(product.discount) || 0; // Agar discount undefined bo‘lsa, 0 bo‘ladi
    const discountPrice =
      discount > 0
        ? product.price - (product.price * discount) / 100
        : product.price;
    return acc + discountPrice * product.amount;
  }, 0);
                ...
                cart.map((product: ICartProduct) => (
                  ...
                  >
                    <td className="px-3 py-4 flex items-center gap-3">
                      <img
                        src={
                          import.meta.env.VITE_BASE_IMAGE_URL + product.image[0]
                        }
                        alt={product.title}
